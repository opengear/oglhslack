...
class LighthouseApiClient:
    ...
    def get_pending(self):
        body = self.get('nodes', { 'config:status' : 'Registered' })
    ...
    def get_enrolled(self):
        body = self.get('nodes', { 'config:status' : 'Enrolled' })
        names = []
        for node in body['nodes']:
    ...
    def get_port_labels(self, node_name):
        body = self.get('nodes', { 'config:name' : node_name })
        labels = []
        for node in body['nodes']:
            for port in node['ports']:
    ...
    def get_summary(self):
        body = self.get('stats/nodes/connection_summary')
    ...
    def _delete_node(self, node):
        body = self.delete('nodes', node['id'])
        return True
    ...
    def _approve_node(self, node):
        ...
        body = self.put('nodes', node['id'], approved_node)
        return True
